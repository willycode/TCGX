...
<jsp:include page="../../home/secure.jsp">
	<jsp:param name="DTREE_PROGRAM_IDENTIFIER" value="SYSAP006" />
</jsp:include>
<jsp:useBean id="obj" scope="request" class="com.kangdainfo.common.view.sys.ap.SYSAP006R">
...
if ("queryAll".equals(obj.getState())) {
	...
	obj = (com.kangdainfo.common.view.sys.ap.SYSAP006R)obj.queryOne();	
}
...
function checkField(){
	...
	if(form1.state.value=="queryAll"){
		alertStr += checkQuery();
		alertStr += checkDate(form1.q_logDate,"起日期");
		alertStr += checkDate(form1.q_logDateE,"迄日期");
		alertStr += checkDateSE(form1.q_logDate,form1.q_logDateE,"起迄日期");
		alertStr += checkTime(form1.q_logTime,"起時間");
		alertStr += checkTime(form1.q_logTimeE,"迄時間");
		alertStr += checkTimeSE(form1.q_logTime,form1.q_logTimeE,"起迄時間");
...
function checkTimeSE(st,en,name){
	if(st!=null&&en!=null&&name!=null){
		if(st.value>en.value){
			st.style.backgroundColor=errorbg;
			en.style.backgroundColor=errorbg;
			 return name+"不合邏輯!!\n";
		}
	}
	return '';
...
function checkTime(obj,name){
	if(obj!=null&&obj.value.length>0&&name!=null&&name.length>0){
		if(isNaN(obj.value) || obj.value.length<6){
			obj.style.backgroundColor=errorbg;
			 return name+"輸入格式錯誤!!\n"
		}else{
			if(obj.value.substring(0,2)>23||obj.value.substring(2,4)>59||obj.value.substring(4)>59){
				obj.style.backgroundColor=errorbg;
				 return name+"輸入格式錯誤!!\n"
			}
		}
...
function doResizing() {
	document.getElementById('listContainer').style.width = document.body.clientWidth-10;
}
</script>
</head>
...
<div id="queryContainer" style="width:500px;height:300px;display:none">
	...
	<tr>
		...
		<td class="queryTDInput">
			<input class="field_Q" type="text" name="q_logTime" size="6" maxlength="6" value="<%=obj.getQ_logTime()%>">~<input class="field_Q" type="text" name="q_logTimeE" size="6" maxlength="6" value="<%=obj.getQ_logTimeE()%>">(輸入格式:HHMMSS)
		</td>
	</tr>
	<tr>
...
<table class="table_form" width="100%" cellspacing="0" cellpadding="0">
	...
	<tr>
		...
		<th class="listTH"><a class="text_link_w" onClick="return sortTable('listTBODY',1,false);" href="#">執行日期</a></th>
		<th class="listTH"><a class="text_link_w" onClick="return sortTable('listTBODY',2,false);" href="#">時間</a></th>
	...
	//String[] alignArray = {"center","center;width 100px","center;width 100px","center;width 100px","left;width 150px","center;width 100px","center;width 100px","center;width 100px","center;width 100px"};
