...
if (list!=null && list.size()>0) {
	for (int i=0; i<list.size(); i++) {
		...
		sb.append("insert into COMMON_CODEKIND(codeKindId,codeKindName) values('" ).append( obj.getCodeKindId() ).append( "','" ).append( obj.getCodeKindName()).append("');<br>");
		if (obj.getCommonCodes()!=null && obj.getCommonCodes().size()>0) {
			for (Object dtlObj : obj.getCommonCodes()) {
				CommonCode dtl = (CommonCode) dtlObj;
				dt.append("insert into COMMON_CODE(codeKindId,codeId,codeName,codeCon1,codeCon2,codeCon3,codeMemo) values(").append(i+1).append(",'").append(dtl.getCodeId()).append("','").append(dtl.getCodeName()).append("'");
